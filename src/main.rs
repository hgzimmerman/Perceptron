...
impl Perceptron {
    fn new(learning_rate: f64, epochs: isize) -> Perceptron {
        Perceptron {
    ...
    fn predict(&self, prediction_vector: &Vec<f64>) -> f64 {
    ...
    fn train(&mut self, training_input_set: &Vec<Vec<f64>>) {
        // TODO this just needs to create a vector (or maybe array) of length 1 less then the provided inputs
        //TODO, optomize this. this is stupid
        let sample = &training_input_set[0];
        // ignore the last element
        self.weights = sample[0..sample.len()-1].iter().map(|_| 0.0).collect(); // set all weights in self to 0.0
...
mod tests {
    ...
    fn training_test() {
        ...
        perceptron.train(&vec!(vec!(0.0, 0.0, -1.0), vec!(1.0, 1.0, 1.0), vec!(1.0, 0.0, -1.0), vec!(0.0, 1.0, -1.0)));
        println!("{:?}", perceptron);
    }
    ...
    fn train_and_predict_invert() {
        let mut perceptron: Perceptron = Perceptron::new(0.1, 5);
        perceptron.train(&vec!(vec!(-1.0, 1.0), vec!(1.0, -1.0)));
